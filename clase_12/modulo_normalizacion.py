...
def normalizacion():
    #Crear una lista de canciones normalizadas.
    canciones = list()

    #Iterar sobre la lista de diccionarios original.
    for cancion in playlist_lady_gaga:
        titulo = cancion["Tema"]
        colaboradores = list()

        nombre_tema_arr = titulo.split(" - ")
        if len(nombre_tema_arr) > 1:
            titulo = nombre_tema_arr[1]
            colaboradores_aux = nombre_tema_arr[0].split(" | ")
            colaboradores.append(colaboradores_aux[0])
            colaboradores.append(colaboradores_aux[1])
        else:
            titulo = nombre_tema_arr[0]
            cancion["Colaboradores"] = "N/A"

        vistas_str = cancion["Vistas"].replace(" millones", "")
        vistas = int(vistas_str) * 1000000

        duracion_arr = cancion["Duracion"].split(":")
        duracion = int(duracion_arr[0]) * 60 + int(duracion_arr[1])

        fecha_lanzamiento_arr = cancion["Fecha lanzamiento"].split("-")
        fecha_lanzamiento = date(int(fecha_lanzamiento_arr[0]),
                                 int(fecha_lanzamiento_arr[1]),
                                 int(fecha_lanzamiento_arr[2]))
        
        canciones.append({"Titulo": titulo, "Colaboradores":colaboradores,
                          "Vistas": vistas, "Duracion": duracion, 
                          "Link": cancion["Link Youtube"],
